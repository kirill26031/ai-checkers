...
//		System.out.println("Added memory: " + (current_memory_used - memory_used));
    ...
    public Move evaluate(int max_depth, long deadline) {
        long start_time = System.currentTimeMillis();
        double returned = root.evaluate(max_depth, deadline);
        if(Double.isNaN(returned)) return null;
    ...
    public void updateByMove(Move move, boolean side) throws IllegalStateException {
        if (root.getChildren().get(0).getChildren().isEmpty() || root.getChildren().get(0).getChildren().get(0).isMax() == side)
            return;
        ...
        root.getChildren().get(0).evaluate(1, Long.MAX_VALUE);
        for (MinMaxVertex child_of_moved : vertex_of_move.getChildren()) {
            child_of_moved.setFather(root.getChildren().get(0));
        }

//        long start = System.currentTimeMillis();
//        int prev_size = leaves.size() + deepest_leaves.size();
//        deepest_leaves = deepest_leaves.stream()
//                .filter(l -> (l.getFather() != null))
//                .collect(Collectors.toCollection(ArrayList::new));
//        ArrayList<MinMaxVertex> new_leaves = new ArrayList<>(leaves.size()/7);
//        int new_leaves_index = current_leaves_index;
//        for(int i=0; i<leaves.size(); ++i){
//            if(leaves.get(i).getFather() != null){
//                new_leaves.add(leaves.get(i));
//            }
//            if(i==current_leaves_index) new_leaves_index=new_leaves.size();
//        }
//        current_leaves_index = new_leaves_index;
//        if(current_leaves_index >= leaves.size()){
//            deepest_leaves.addAll(new_leaves);
//            leaves = deepest_leaves;
//            deepest_leaves = new ArrayList<>(leaves.size()*8);
//            current_leaves_index = 0;
//        }
//        else leaves = new_leaves;
//        System.out.println("For removing redundant elements from leaves and deepest_leaves spent: " +
//                (System.currentTimeMillis() - start) + "(ms)");
//        System.out.println("Size reduced from " + prev_size + " to " + (leaves.size() + deepest_leaves.size()));
    }
